...
while true; do
  ...
  if [[ ! -f "$FILE_PATH" ]]; then
    if ! ffmpeg -framerate "$VIDEO_24_TIMELAPSE_INPUT_FPS" -i "$LATEST_NOT_PROCESSED_PATH" -c:v libx264 -r "$VIDEO_24_TIMELAPSE_OUTPUT_FPS" -pix_fmt yuv420p "$FILE_PATH"; then
      continue
    fi
  else
