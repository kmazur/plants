...
function update_daylight_info() {
  ...
  if [[ "$STATUS" == "OK" ]]; then
    ...
    local FIRST_LIGHT="$(echo "$JSON_PAYLOAD" | jq -r '.results.sunset')"
    local LAST_LIGHT="$(echo "$JSON_PAYLOAD" | jq -r '.results.sunset')"
    local DAWN="$(echo "$JSON_PAYLOAD" | jq -r '.results.sunset')"
    local DUSK="$(echo "$JSON_PAYLOAD" | jq -r '.results.sunset')"
    local SOLAR_NOON="$(echo "$JSON_PAYLOAD" | jq -r '.results.sunset')"
    local GOLDEN_HOUR="$(echo "$JSON_PAYLOAD" | jq -r '.results.sunset')"
    local DAY_LENGTH="$(echo "$JSON_PAYLOAD" | jq -r '.results.sunset')"
